...
class SaveAndDownloadUserPdf {
  static createPdf({
    required final UsersType users,
...
//get attendance
    ...
    //attended event
    List attendedEvent = getEventAttendedList(users.eventAttended);
    ...
    //penalty conversion
    final penalty =
        totalValue >= 100 ? penaltyFormula[100] : penaltyFormula[totalValue];

    var data = events.map((e) {
      return [
        e.eventName.toString(),
        DateFormat('MMMM dd, yyyy').format(e.eventDate),
        getAttendanceList(users.eventAttended, e.id, e.eventPenalty)
      ];
    }).toList();
                  ...
                  children: [
                    pw.Container(
                      ...
                      child: pw.Row(
                          ...
                          children: [
                            pw.Image(pw.MemoryImage(imgCite),
                                height: 60, width: 60),
                            pw.Column(children: [
                            ...
                            pw.Image(pw.MemoryImage(imgNDMC),
                                height: 60, width: 60),
                          ]),
                    ),
                    pw.Container(
                        ...
                        children: [
                          ...
                          pw.Column(
                            ...
                            children: [
                              pw.Text(''),
                              pw.Text(
                                  'Course: ${users.userCourse}-${users.userYear}')
                            ],
                          )
                        ],
                      ),
    ...
    final filename = '${users.userName}_${users.lastName}_attendance.pdf';
